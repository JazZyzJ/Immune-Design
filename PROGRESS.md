...
- **Uricase case study (standalone, not in main test set).** Files under `uricases/`:
  ...
  - **Tetramer interface gate (2026-07-20, code ready + CPU-validated)** — torch-free reusable metrics in `inverse_folding/evaluation/tetramer_interfaces.py`; `scripts/eval_tetramer_gate.py` scores all six protein chain pairs after ESMFold2/Protenix prediction, classifies the D2 graph into two repeated biological interfaces plus diagonals, and writes both a variant summary and chain-pair long table; `submit_tetramer_eval.slurm` is the CPU/Rosetta launcher. Always-on metrics are coordinate BSA (total + per partner), residue contacts, salt bridges, and topology separation; optional Rosetta 3.15 InterfaceAnalyzer adds dSASA, dG/dSASA, packstat, H-bonds/energy fraction, BUNS, and shape complementarity for all four interface copies. Final-selection mode uses `--require-interface-wt` (real same-backend WT only; no placeholder) and conservative weaker-copy summaries for both interfaces. 1R51 validation: coordinate BSA ≈5.86k/5.20k Å² total (2.93k/2.60k per partner), diagonal ≈0.74k; real Rosetta batch and one-design Protenix end-to-end smoke both pass. Threshold calibration remains pending assembly/activity anchors; protocol = `PROTOCOL/tetramer_prediction_gate.md`.
