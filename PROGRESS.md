...
- **Uricase case study (standalone, not in main test set).** Files under `uricases/`:
  ...
  - `pdbs_if_ready/` — structure files retained on disk (6740: 5222 `.cif` + 1518 `.pdb`; 353 orphaned exact-dups kept) + `Pegloticase.pdb` (AF3); `h_maps/h_maps_uricase_DRB1_{07_01,04_01}.parquet` — **6388** each (Pegloticase concatenated; 0 missing / 0 md5-mismatch vs unified), covers 100%, npoff; `wt_generated_uricase_caseset_HLA-DRB1_07_01.parquet` facade — 6387 (pre-Pegloticase).
