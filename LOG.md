...
### L0154
- timestamp: 2026-08-10T21:15:51-04:00
- type: FIX
- module: RF/FUSION_V2
- trigger: The high-risk D2/D8 ceiling campaign requires four independent roots per protein, while the ordinary ladder hardcoded one runtime sampler seed, `pair_ordinal=0`, and `r0/fam0` across every invocation.
- change_summary: Added an explicit `root_index={0,1,2,3}` execution identity from CLI/SLURM through seed derivation, root capture, lineage, resume fragments, journals and manifests; D2/D8 with the same root index share the exact D0 pool, while distinct roots cannot collide or overwrite one another.
- rationale: Depth is the treatment, so D2 and D8 must start from the same random D0 pool at a given replicate; replicate identity must nevertheless change both the fresh-root seed and every descendant pair namespace. The root seed is now exactly `V2SeedContext.depth0_root_seed(c0, root_index)`, and `pair_ordinal=root_index`. Explicit roots use `rN/famN`, bind ordinal plus realized per-protein seed into `RunSignature`, isolate fragments and journals under `root_NNNN`, and the launcher isolates the complete bundle under `<CELL>/root_NNNN`. Omitting `--root-index` preserves the historical sampler stream, signature canonical bytes, fragment name, journal path, manifest and shard-payload shape.
- artifacts:
  - `inverse_folding/reference_flow/fusion_v2_runtime/{capture,ladder}.py`
  - `scripts/{rf_fusion_v2_oracles,rf_fusion_v2_cohort,rf_fusion_v2_resume,rf_fusion_v2_artifacts,run_rf_fusion_v2}.py`
  - `scripts/submit_rf_fusion_v2_canary.slurm`
  - `doc/SCRIPTS.md`
- evidence: TDD red at the absent `_root_lineage_identity` boundary; focused root/capture/ladder/driver/cohort/oracle/artifact gate `281 passed`; adjacent identity/seed/cycle/paired/mechanism/preflight/materializer regression gate `372 passed`; `py_compile`, `bash -n scripts/submit_rf_fusion_v2_canary.slurm`, and `git diff --check` pass.
- impact:
  - scope: additive for explicit ordinary-ladder roots. Mechanism/qualification mode refuses `root_index` because `source_index` already owns that replicate axis.
  - risk: low -- all legacy fields and paths are conditionally omitted rather than defaulted to an explicit root zero.
  - confidence: 0.97
- status: done
- next_action: Materialize D2 and D8 under one campaign/split/master seed and common `c0`, then launch `ROOT_INDEX=0..3` for both profiles so each index shares its D0 pool across depth profiles.

### L0155
- timestamp: 2026-08-10T23:36:00-04:00
- type: FEATURE
- module: RF/FUSION_V2
- trigger: The formal DRB1*07:01 high-risk capability-ceiling campaign needed one executable method contract before D2/D8 generation, with WT excluded as an optimization incumbent and the stochastic Gumbel DPLM-native run fixed as the comparator.
- change_summary: Added the frozen D2/K32 and D8/K32 high-risk profiles, logical D0 best-admissible bootstrap, dual scTM ancestry/strict gates, exact R4 lineage identity, typed complete-negative outcomes, content-bound Head/ESMFold2 runtime identities, cache-execution accounting, semantic launch identity over actual execution aliases and resolved backbones, strict clean-Git launch verification, an exact live Gumbel baseline evaluator, and an independently consumable dual-R4 archive facade.
- rationale: A capability result is uninterpretable if deeper cells start from different D0 pools, WT is silently used as the reward baseline, search ancestry is conflated with final strict feasibility, cached folds are charged as model calls, or signed paths are not the files actually loaded. These guards preserve the aggressive search while making its terminal frontier and zero-strict outcomes auditable.
- artifacts:
  - `inverse_folding/reference_flow/fusion_v2/{config,identity,policy,reward,safety,state,structure_gate}.py`
  - `inverse_folding/reference_flow/fusion_v2_runtime/{admission,archive,capture,cycle,ladder}.py`
  - `inverse_folding/evaluation/esmfold2_live.py`
  - `scripts/{materialize_v2_canary_config,run_rf_fusion_v2,rf_fusion_v2_cohort,rf_fusion_v2_resume,rf_fusion_v2_artifacts,rf_fusion_v2_oracles,materialize_v2_archive_facade,evaluate_rf_fusion_v2_exact_live_baseline}.py`
  - `inverse_folding/reference_flow/configs/v2_head_directed_capped_policy_v2.json`
  - `doc/FUSION_V2.md`, `doc/SCRIPTS.md`
- evidence: Focused structure/materialization/exact-baseline gate `97 passed`; driver/resume identity gate `120 passed`; facade/cohort/complete-negative gate `220 passed`; `py_compile` and `git diff --check` pass.
- impact:
  - scope: explicit `exploratory_highrisk_ceiling_v1` D2/D8 profiles only; legacy V2 signatures and ordinary single-root execution remain byte-compatible when the new identity fields are omitted.
  - risk: medium -- this opens a large unblinded capability search, but launch remains fail-closed on method, content, runtime and source-tree drift.
  - confidence: 0.94
- status: implementation frozen locally; cluster calibration and longest-protein canary pending.
- next_action: Commit the frozen implementation, calibrate the C=454 Head policy and per-protein full-trajectory hotspot thresholds, materialize all protein-bound cells, then run the longest-protein D8/root0 canary before the eight cohort tasks.
