...
- type: FEATURE
- module: INFRA
- trigger: Della's RTX PRO 6000 Blackwell node is compute capability 12.0, while several frozen result-producing environments used PyTorch/CUDA builds without `sm_120` kernels. Every active project GPU runtime needed an explicit Blackwell execution path without modifying the reproducible H100/H200 environments.
- change_summary: Completed RTX PRO 6000 adaptation for all active environments. (1) Immune-Design uses the isolated `immune-design-blackwell` clone with `torch 2.7.1+cu128`, matching torchvision/torchaudio, Triton 3.3.1, and `torch_scatter 2.1.2+pt27cu128`; DPLM's trusted checkpoint loader explicitly preserves the pre-PyTorch-2.6 `weights_only=False` contract, and `submit_if_phase_c.slurm` accepts opt-in `CONDA_ENV`. (2) CanonFlow uses the isolated `equinv-blackwell` clone with the CUDA 12.8 overlay and `CANONFLOW_CONDA_ENV`, while `equinv` remains frozen. (3) The existing owner-only `esmfold2` environment already uses `torch 2.12.0+cu130` and contains `sm_120`, so it required no duplicate environment; its supported monomer/refold launcher now permits an RTX partition/GRES override. (4) Protenix uses the owner-only `tools/protenix/envs/v2.0.0-blackwell` with `torch 2.7.1+cu128`; the original `v2.0.0+cu126` remains the H100/H200 default, while project and shared launchers accept explicit `PROTENIX_ENV` plus RTX resource overrides. Reproducible CUDA 12.8 overlay locks and usage documentation were added for each cloned environment.
- evidence: On `della-h23g1` (RTX PRO 6000 Blackwell Server Edition, 97,887 MiB, driver 610.43.02), `immune-design-blackwell` reported `(12, 0)` and `sm_120/compute_120`; BF16 backward, CUDA `torch_scatter`, `torch.compile`/Triton, a 3.25 GB DPLM checkpoint with 119/119-residue generation, and real a1res03 Head inference passed, together with 127 focused tests. CanonFlow's `equinv-blackwell` passed BF16 backward, `torch.compile`, SemlaGenerator forward/backward, and real QM9 checkpoint evaluation; G-A0/G-A1/G-A2/Q-C1 jobs `11778754/11778752/11778753/11778742` all completed `0:0` on `rtx6000`. ESMFold2 passed BF16/Triton and a real `biohub/ESMFold2` 27-aa fold, writing CIF/PDB. The old Protenix cu126 environment failed its first RTX CUDA operation; the cu128 environment passed `pip check`, all 5 upstream installation tests, BF16/Triton/cuequivariance, strict 464.44M-checkpoint loading, no-MSA inference, and the default local-GPU-MSA workflow in job `11853601` (`0:0`, MSA depth 191, five CIF/confidence samples, 18.72-second model forward). Repository-side ESMFold2/Protenix regression tests passed 23/23, and Slurm dry-runs placed both launchers on `della-h23g1`.
- impact: All active project GPU environments now have a validated, explicit RTX PRO 6000 path: `immune-design-blackwell`, `equinv-blackwell`, the existing `esmfold2`, and Protenix `v2.0.0-blackwell`. Original `immune-design`, `equinv`, ESMFold2 H200 behavior, Protenix `v2.0.0`, and all H100/H200 defaults remain unchanged. No scientific result or checkpoint was altered.
- status: done
- refs:
  - `requirements-blackwell-cu128.txt`, `DELLA_USAGE.md`, `scripts/submit_if_phase_c.slurm`, `scripts/submit_esmfold2_gt.slurm`, `scripts/submit_protenix_refold.slurm`, `scripts/submit_tetramer_predict.slurm`
  - `inverse_folding/dplm/src/byprot/tasks/lm/dplm_invfold.py`, `tests/inverse_folding/test_dplm_checkpoint_compat.py`, `tests/scripts/test_esmfold2_precompute.py`
  - `/home/zc1519/src/CanonFlow/repo/semla-flow/{README.md,requirements-blackwell-cu128.txt}`, `/home/zc1519/src/CanonFlow/repo/report/EXPERIMENT_TRACKER.md`
  - `/scratch/gpfs/KAIYIJIANG/tools/protenix/docs/{README.md,requirements-blackwell-cu128.txt}`

### L0139
- timestamp: 2026-07-31T22:58:00-04:00
- type: FEATURE
- module: INFRA
- trigger: PD1_b2 binder de-immunization v2 needs binding decided on a re-predicted COMPLEX, but the whole post-prediction interface stack was hardwired to the uricase D2 homotetramer (`len(pchains) != 4` raised, `classify_d2_interface_pairs` required 6 pairs, `summarize_interface_pairs` required two symmetry copies plus a diagonal, and `tier1_from_summary` indexed a 4x4 slice unconditionally). A binder:target hetero-dimer could not be scored at all.
- change_summary: Generalized the interface stack to two complex topologies and renamed it accordingly: `inverse_folding/evaluation/tetramer_interfaces.py` -> `complex_interfaces.py`, `scripts/eval_tetramer_gate.py` -> `eval_complex_gate.py`, `scripts/submit_tetramer_eval.slurm` -> `submit_complex_eval.slurm`. Added `classify_interface_pairs()` dispatching on chain count (4 -> the unchanged D2 path; 2 -> a single `interface_1` with no symmetry copy and no diagonal; anything else fails fast), made `summarize_interface_pairs()` drop the two-copy contract and the topology block when no diagonal exists, and clamped `tier1_from_summary()`'s `n_protein` to the confidence-matrix size. Added `scripts/build_binder_complex_inputs.py` (RF cohorts -> per-design query-only binder a3m + prediction manifest + WT-carrying gate manifest) and `inverse_folding/reference_flow/configs/pd1_binder_interface_v2.yaml` (57 hard anchors: the 24 contact anchors plus the 33 support-shell residues promoted from `monitored_posthoc`). Built and installed `USalign` at `/scratch/gpfs/KAIYIJIANG/tools/common/bin/USalign`, which was absent (`complex_TM_vs_wt` was silently returning NaN).
- rationale: A binder's function is its interface, and an apo monomer refold places interface side chains with no partner to order them, so no monomer metric is evidence about binding (`PROTOCOL/binder_interface_deimm_selection.md`). Everything below the chain-pair classifier -- BSA, contacts, salt bridges, and the Rosetta InterfaceAnalyzer dimer layer -- was already topology-agnostic; only the classifier and three guards were uricase-specific, so generalizing beat forking. The v2 manifest exists because the v1 cohort proved selection is not a substitute for constraint: its strictest structural survivors still mutated 17/33 (0401) and 15/33 (1501) shell positions (Mann-Whitney p=0.827 / p=0.003 with a one-position effect).
- artifacts:
  - `inverse_folding/evaluation/complex_interfaces.py`, `scripts/eval_complex_gate.py`, `scripts/submit_complex_eval.slurm`
  - `scripts/build_binder_complex_inputs.py`, `inverse_folding/reference_flow/configs/pd1_binder_interface_v2.yaml`
  - `tests/scripts/test_eval_complex_gate.py`, `doc/SCRIPTS.md`
- evidence: 633 tests pass under `tests/scripts/` (two pre-existing collection errors are unrelated: `scripts/build_tetramer_input.py` no longer exists and `scripts/analysis/` is gitignored). Five new tests cover the hetero-dimer classifier, the degenerate summary, the two-chain `interface_pair_metrics` path, the rejection of unsupported chain counts, and the `tier1` clamp; one is an explicit regression guard asserting `classify_interface_pairs(_pair_table()).equals(classify_d2_interface_pairs(_pair_table()))` so the D2 path stays byte-identical. End-to-end smoke on the real PD1_b2 parent complex with Rosetta 3.15: 1 chain-pair row, iptm 0.950733, plddt 93.57, has_clash False, BSA 2486.6 A2, 85 contacts, 3 salt bridges, InterfaceAnalyzer dSASA 2495.7 A2 / packstat 0.555 / hbonds_int 12 / SC 0.643 / BUNS 11, and every metric carried a real `_ratio_to_wt` / `_delta_vs_wt` column under `--require-interface-wt`. `USalign -mm 1` self-alignment = 1.00000, parent intra-ensemble sample-to-sample = 0.99371. The v2 manifest round-trip-validates against the 114-aa parent (`validate_against_sequence` PASS, hash `aaaa5388b948`), and `refine_rf_designs.py::_load_catalytic_indices_by_protein` returns the 24 contact indices via a new `annotation_provenance` block rather than the whole 57 (support-shell residues are frozen for safety, not because they are functional).
- impact:
  - scope: every consumer of the post-prediction interface stack; the four-chain uricase behavior is unchanged by construction and by regression test. `eval_tetramer_reference.py` keeps its name (it IS tetramer-specific) and only follows the module rename.
  - risk: medium
  - confidence: 0.88
- status: done
- next_action: read the v2 complex-gate cohort once it lands and calibrate B1/B2 against the parent row rather than against borrowed AF2 thresholds.
- refs:
  - `PROTOCOL/binder_interface_deimm_selection.md`, `cases/pd1b2_binder_deimm_v2_20260731.yaml`
  - Rosetta caveat recorded for the reader: `dG_separated` on an UNREPACKED predicted binder complex is +9.45 REU for the parent itself (`-pack_input false -pack_separated false`), so its absolute sign is a no-repack artifact and only the `_delta_vs_wt` form is interpretable on this target.

### L0140
- timestamp: 2026-08-03T18:20:00-04:00
- type: FEATURE
- module: RF/CONSTRAINTS
- trigger: Audit of the 44 Active-15 core-release constraint manifests found two gate defects. (1) The frozen audit only proved that C80/C90/contact/energy indirectly capture every homolog-projected functional-analog collision **inside P1+P4**; the builder opens P1/P4/P6/P9, and 7 monitored-shell analogs (the Q00511 `Val228` family, all at P9) escaped every gate and entered the free set. (2) `in_energy_ddg_gt0_reu` was a hard veto, but the alanine scan runs on an un-relaxed predicted ensemble: WT `ddG_bind` reaches -371.78 REU (catalytic) / -83.98 REU (assembly) at cross-chain heavy-atom distances of 1.20 / 1.00 A, i.e. Rosetta clash blow-ups, and `corr(distance, ddG)` is +0.221 / +0.135 -- so the sub-1-REU band of that veto is noise, not WT instability.
- change_summary: Added `--lock-functional-analogs` (promotes `homolog_analog_overlap | legacy_identity_required_overlap` from annotation to hard veto, new blocked reason `functional_analog_overlap`) and `--energy-ddg-tier {gt0,ge1,ge2}` to `scripts/build_uricase_active15_core_release.py`, plus `--n-designs-per-protein` which overrides only the launch-manifest design count and never mutates the frozen `c1_null` sampler config. Locked functional positions now emit `biological_role: functional_analog_lock` with a reference-frame label instead of collapsing into the undifferentiated `near_WT_complement_lock`, and each manifest's `annotation_provenance` gained a computed `gate_policy`, `functional_analog_hard_lock`, `n_functional_analog_locks`, and an explicit `numbering_contract`. `PARENT_COLUMNS`/`CORE_COLUMNS`/`PHYSICAL_POSITION_CONSISTENCY_COLUMNS` were extended to the columns the new gates read. Published `05_core_release_v2` with `--lock-functional-analogs --energy-ddg-tier ge1 --n-designs-per-protein 150`.
- rationale: Locking the analogs is free -- it removes 7 positions that no core depends on, so 0 cores and 0 cells are lost while the fixed fraction rises. The energy tier is nearly free in the other direction: `full_contact_5of5` already subsumes 376/384 (97.9%) of the `gt0` anchor hits, so `ge1` recovers only +2 cores; it is taken as hygiene (dropping clash-noise-driven vetoes), not as a search-space lever. The real lever -- `full_contact_5of5` -> `mid_contact_5of5`, measured at 184 -> 207 cores with the fixed-fraction median falling only 0.9689 -> 0.9636, and where all 38 newly released positions have negative assembly `ddG` and zero catalytic-5-of-5 membership -- was deliberately NOT taken: the user is holding the most conservative gate until a design with measurable activity exists.
- artifacts:
  - `scripts/build_uricase_active15_core_release.py`, `tests/scripts/test_build_uricase_active15_core_release.py`, `doc/SCRIPTS.md`
  - `PROTOCOL/tetramer_prediction_gate.md` (new "Frame contract" section + rewritten Numbering principle), `PROTOCOL/INDEX.md`
  - `/scratch/gpfs/KAIYIJIANG/zijie/work/immune-design/therapeutic_enzymes/uricase_active15_ev_structure_v1/05_core_release_v2/`
- evidence: v2 published 44 manifests / 186 openable cores, matching the pre-computed target exactly. Independent re-derivation of the gate from the raw evidence tables reproduced all 1092 anchor decisions with 0 blocked-flag and 0 blocked-reason disagreements, and 0 released-but-blocked collisions. v1 -> v2 diff is exactly the intended one: the 7 predicted `Val228`-family analogs moved released -> locked, 5 positions with `0 < ddG < 1` moved locked -> released, and **0 cores were lost** (184 -> 186 openable, 426 -> 424 free positions, 44 -> 44 emitted cells, fixed-fraction median 0.9689 -> 0.9670, minimum unchanged at 0.9329). Full index audit passes: `wt_aa == sequence[index_0b]` for 2457/2457 core rows and 4548/4548 parent rows (the 1-based misread scores only 4.4%, so the check discriminates); `hard_anchors | free_positions` is an exact partition of `[0, L)` with `index_0b: 0` locked in all 44; anchor labels are 1-based while every index is 0-based; all 15 AFDB CIFs have `auth_seq_id` 1..L and character-identical chain sequences; 135 homolog-analog annotations are index/AA self-consistent. 9/9 builder tests pass, including a new one covering both flags.
- impact:
  - scope: `05_core_release_v2` supersedes `v1` as the launch bundle; `v1` is retained unmodified for provenance. The two flags default to the legacy behavior, so no other caller changes.
  - risk: low
  - confidence: 0.93
- status: done
- next_action: RF-only generation at n=150 per cell (no refinement this round). The 9 cells with <=4 free positions (min 1: `Q7SBV5x15:01`, `Q9RV70x04:01`) saturate far below 150 and should be sized down or de-duplicated post-hoc.
- refs:
  - Frame contract frozen in `PROTOCOL/tetramer_prediction_gate.md`: generate full-length with the initiator Met hard-locked, predict/compare in the mature frame at `--crystal-offset 0`, order full-length with Met. Met excision is NOT universal -- `A0A9P8P4R1` (residue 2 = L) and `Q9RV70` (residue 2 = M) must never be Met-stripped.
  - Numbering identity: for a Met-excised parent, manifest `index_0b` equals the mature-frame 1-based author residue id, which is exactly `eval_complex_gate.py`'s `CATALYTIC_CORE` values -- no offset bookkeeping.

### L0141
- timestamp: 2026-08-03T22:40:00-04:00
- type: FEATURE
- module: RF/LAUNCH
- trigger: The v2 launch manifest carried an empty `controller_config`, no epitope head, and `n_designs_per_protein` copied verbatim from the frozen `c1_null` sampler (=1), so it could not be executed as-is. Submitting through `scripts/submit_if_phase_c.slurm` was rejected after auditing its defaults: five variables silently resolve to values that are wrong for this cohort -- `ALLELE` -> `HLA-DRB1*07:01` (every cell would run as 07:01), `CONSTRAINT_MANIFEST` -> `""` (completely unconstrained), `H_MAPS_PARQUET` -> an allele-wide `h_maps_v2` file (its `${VAR-default}` form fires whenever the variable is merely unset), `TEST_SET_PARQUET` -> `fast_v2_<tag>`, and `CONDA_ENV` -> the non-sm_120 environment.
- change_summary: Added the launch layer to `scripts/build_uricase_active15_core_release.py`: `--controller-config`, repeatable `--head-checkpoint ALLELE_TAG=PATH`, `--head-config-dir/-variant-id/-device/-window-batch-size/-allele-idx`, `--n-designs-per-free-position` (per-cell cap `min(base, cap * n_free)`), and `--run-id-prefix`. A pre-publish gate now refuses to emit when a controller is given without exactly one head per emitted allele, when a head is supplied for a non-emitted allele, or when two alleles share a head checkpoint. Launch rows gained `controller_config[_sha256]`, `head_checkpoint[_sha256]`, `head_config_dir`, `head_variant_id`, `head_device`, `head_window_batch_size`, `head_allele_idx`; `metadata.definitions` gained per-allele design counts, the total, the cap, the controller path, and the head map. Added the run driver `run/.../uricase_active15_core_release_v2/driver/{submit_gen_array.slurm,verify_runs.py}`, which calls `run_if_phase_c1.py` directly (its critical arguments are `required=True`, so a bad value aborts instead of resolving to a default) and re-asserts head/allele agreement and the `output_root/allele/run_id` composition per task.
- rationale: The launcher's convenience defaults are a silent-failure surface for a 44-cell heterogeneous sweep where allele, cohort, and constraint manifest all vary per task; `required=True` on the driver is a louder contract than `${VAR:-default}`. The head must be per-allele because the controller's immune signal is allele-specific -- a mismatch optimizes the wrong allele while every log line still looks correct, so it is enforced at build time, again in the driver, and verified a third time post-run from each run's manifest.
- artifacts:
  - `scripts/build_uricase_active15_core_release.py`, `tests/scripts/test_build_uricase_active15_core_release.py`, `doc/SCRIPTS.md`
  - `PROGRESS.md` (new Active-15 core-release v2 section)
  - `/scratch/gpfs/KAIYIJIANG/zijie/run/inverse_folding/therapeutic_enzymes/uricase_active15_core_release_v2/driver/`
- evidence: Array `11971273` -- **44/44 COMPLETED, 0 failures, 9.55 GPU-h, 5610 designs -> 4910 unique (87.5%)**. Post-run verification against every run's own `manifest.json` passed 12/12 checks on 44/44 runs: `controller_config_hash == 2c39f229...` (byte-identical to the PD1/ADA/ADI/Q00511 cases), `controller_mode == d2_d3_full`, head path contains the allele's own tag, `head_variant_id == LC1`, `constraint_manifest_hash` matches the launch row, `enzyme_mode_enabled`, `all_anchors_preserved` with `num_anchor_mismatches == 0`, `n_rows_generated == n_designs_per_protein`, `h_maps_source_path is None`, `allele == runtime_allele`, seed, and an independent per-sequence re-check that every hard anchor still equals its `expected_aa` in all 5610 generated sequences. Pre-submission dry-run validated all 44 manifest rows for head/allele, controller identity, constraint-manifest match, run-dir composition, run-id prefix, and runtime-allele form (0 anomalies). 9/9 builder tests pass. The rebuild that added `--run-id-prefix` left the 44 constraint manifests byte-identical (aggregate SHA-256 `68a63970...` before and after), proving the change touched only the launch layer.
- impact:
  - scope: launch-manifest schema gained 8 columns; all new flags default to the previous behavior, so other callers are unaffected. The constraint layer is untouched.
  - risk: low
  - confidence: 0.95
- status: done
- next_action: NMP immune + monomer structure evaluation on the 4910 unique designs.
- refs:
  - Diversity is governed by commit rate, not free-position headroom: the controller touches a median 92.9% of free positions, and `mut_per_design` (median 4.21) predicts unique output better than `n_free`. Two `free=11` cells at `mut_per_design ~ 2.05` yielded 89/98 unique while `free=8` cells at ~4.0 yielded 142. The `20 x n_free` cap used here is therefore the wrong scaling law for the next round.
  - Degenerate cell: `Q7SBV5 x 15:01` (free=1, `mut_per_design` 0.35) produced 2 unique from 20 -- most of its designs are exact WT.

### L0142
- timestamp: 2026-08-04T01:05:00-04:00
- type: FIX
- module: RF/REFINE
- trigger: Preparing full-cohort refinement for the 44 Active-15 cells surfaced three contracts that would each have wasted the whole run. (1) `refine_rf_designs.py::_load_catalytic_indices_by_protein` raises for any non-`_v0` schema lacking `annotation_provenance.direct_functional_union_uniprot_1b`; all 44 manifests are `uricase_active15_core_release_v1`, so the protocol structure gate could not resolve `cat_max_scRMSD` at all. (2) `submit_refine.slurm` requires three protocol-gate thresholds and ships no defaults, and any cross-parent absolute value rejects some parent's own wild type. (3) `normalize_to_cache` wrote the refold cache with a plain `open(path,"w")`, but 44 concurrent jobs share one content-addressed cache and legitimately fold identical candidate sequences.
- change_summary: `scripts/build_uricase_active15_core_release.py` now republishes the locked functional positions as `annotation_provenance.direct_functional_union_uniprot_1b` (1-based UniProt) plus `direct_functional_released_uniprot_1b`; an empty union is emitted as an ABSENT key so a release-everything gate policy stays buildable and fails closed downstream rather than at build time. `inverse_folding/evaluation/refold_normalize.py` gained `_atomic_write_text` (same-directory temp file + `os.replace`) and both cache writes go through it. `PROTOCOL/shortlist_and_refine_seed_selection.md` gained three subsections: cell-specific seed-table dedup, the WT-floor procedure for calibrating the three gate thresholds, and the `STRONG_RANK` fraction-vs-percentage scale contract.
- rationale: The catalytic subset already existed in every manifest as `biological_role: functional_analog_lock`; republishing it under the key the refiner reads adds no new claim and leaves the free/fixed partition untouched. Gate thresholds must be floor-relative because the same-predictor WT floor itself spans `cat_max_scRMSD` 0.556-12.937 A across parents -- a constant margin on top of each parent's own floor is what actually delivers the protocol's "equal search controls", whereas a shared absolute value silently applies unequal strictness. The cache write is atomic because its failure mode is a truncated PDB that surfaces much later as a parse error or a silently wrong structure, not at the point of corruption.
- artifacts:
  - `scripts/build_uricase_active15_core_release.py`, `inverse_folding/evaluation/refold_normalize.py`, `PROTOCOL/shortlist_and_refine_seed_selection.md`, `doc/SCRIPTS.md`
  - `work/.../uricase_active15_ev_structure_v1/05_core_release_v2_catmask/` (44 rebuilt manifests; v2 left byte-identical for generation provenance)
  - `run/.../uricase_active15_core_release_v2/wt_floor/` (44-cell WT floor + metadata), `refine/GATE_CALIBRATION.md`, `refine/seed_tables/` (44 tables + frozen index), `refine/submit_all_cells.sh`
- evidence: Rebuild is provably non-mutating -- all 44 manifests' `entries` blocks are byte-identical to v2, the only new provenance keys are the two added ones, and no existing provenance value changed. `_load_catalytic_indices_by_protein` accepts 44/44, returning exactly 9 indices per cell, all of them protected hard anchors; Q00511 resolves to `[11,58,59,160,177,228,229,255,257]`, i.e. `eval_complex_gate.py`'s `CATALYTIC_CORE`. 9/9 builder tests and 22/22 refold tests pass; a 64-way concurrent write of one key yields a complete file and no leftover dot-files. WT floor computed through the identical `evaluate_phase_c.py --mode struct` path on 44/44 cells (15/15 WT folds were cache hits under `evaluate`'s own `cache_key`, so no re-fold). At margins (0.03, 1.0 A, 15) the gate leaves 95.5% of the 5610 evaluated designs inside and no cell below 50%; the binding term is pLDDT (244) then cat (9), scTM binds 0 times. Seed table is 4910 rows over 44 cells (cell-specific dedup; global dedup would be 4906) with no immune/structure pre-filter. Driver dry-run validates 44/44 rows and a deliberately tampered manifest is caught by the sha256 check (`FATAL: sha256 drift`).
- impact:
  - scope: builder output gains two provenance keys (existing callers unaffected; absent-key path preserves old behavior). `refold_normalize` change is a pure correctness fix used by every ESMFold2 refold path. Constraint semantics, free/fixed partition, and generation artifacts are untouched.
  - risk: low
  - confidence: 0.93
- status: done
- next_action: 109 shards submitted across 44 cells on rtx6000. On completion, merge with `merge_refine_shards.py`, verify per-cell head/allele from each run's own config, then hand the paired WT -> seed -> refined cohort to the tetramer gate.
- refs:
  - `A0A9P8P4R1`'s apparent 12.5 A catalytic displacement is an artifact of `_global_ca_alignment`'s untrimmed full-residue Kabsch fit: its ~26-residue N-terminal extension sits 30-60 A from the AFDB position **in the WT more than in the designs** (WT median 34.4 A vs design 31.1 A; WT `cat_max_scRMSD` 12.94 vs design median 12.50). Refitting on residues 20+ gives design catalytic RMSD 0.55 A vs AFDB and 0.23 A vs WT-ESMFold2. An earlier "excess RMSD" figure for this parent was invalid because it subtracted TMalign's aligned-only RMSD from evaluate's full-residue RMSD.
  - `scTM` is predictor-determined in this near-WT regime: per-parent WT-vs-AFDB TM and design median scTM agree to three decimals, so scTM carries no discriminating power here and acts only as a collapse guard.
  - `predicted_active_site_min_pLDDT` degenerates when the manifest locks ~97% of the chain: it becomes whole-protein minimum pLDDT and lands on a flexible terminus. The catalytic claim rests on `cat_max_scRMSD` over the 9 declared residues.

### L0143
- timestamp: 2026-08-06T02:10:00-04:00
- type: FIX
- module: RF/FUSION_V2
- trigger: Completing V2F7 (production run surface) and V2F8 (local gate) exposed that several V2 contracts were declared but never enforced at runtime. Two adversarial review passes followed -- an external eight-point review, then a six-reviewer workflow over the PLAN task blocks and the FUSION_V2 doc set (211 requirements checked, 33 findings, 7 P1 surviving refutation). The common failure mode: a rule that lives in a docstring, a manifest, or a module nothing calls is not a rule. Every one of the eight would have surfaced only on the cluster, and four of them would have surfaced as plausible numbers rather than as errors.
- change_summary: Made the frozen V2 config the runtime AUTHORITY rather than a manifest label -- for the arm identity, the projection policy, the schedule band, the reference content and the SUBSTRATE -- replaced the projected cost model with realized accounting reconciled against counted forward passes, wired the attempt journal into the execution chain, gave the oracles a typed request domain, content-bound resume fragments to their own results, completed the cohort evidence graph, and closed the six artifact/correctness gaps the second review found.
- rationale: Each fix closes a path by which a green local run could report something it did not do. (1) The Head and structure oracles were handed `sequence_md5` where a model consumes a sequence: an MD5 is still a `str`, so it satisfies every identity-keyed fake and fails only on a real backend, after a GPU allocation is paid for. Both now take a typed `OracleRequest` carrying the residue string with the digest alongside as a cache/join key, verified to key its own sequence. (2) `config.arm.feedback_enabled` reached only the manifest; the cycle defaulted it to `True`, so an A2 config would have run WITH feedback while its own artifact recorded `feedback_enabled: false` -- a treatment arm labelled as its own control. It is now required with no default at both the cycle and the ladder, and the ladder refuses before paying the root prefix. (3) `q_phi` never compared the answering policy or the supplied band table against the run's own frozen conditioning, which carries both digests by value; `gate_projected_maturity` is not that check, because it compares the declared identity against the table it was HANDED, so a self-consistent foreign calibration satisfies it completely. Both are now bound, and the check precedes the policy-rejection branch because a foreign policy's refusal is not this run's refusal either. An answer carrying no identity at all is refused outright -- a gate that can be opted out of by omission is not a gate, and PLAN Sec 5.3 requires every feedback event, null ones included, to name its policy. (4) `CycleCost` was computed before the feedback branch and returned unchanged on every exit path, so a cycle that stopped at the A2 view was billed for a segment and a descendant pool it never ran; the ladder then double-charged an inherited rung's prefix and never charged the deepest rung's descendant pool at all. PLAN Sec 4.3 matches the arms on one declared resource read out of exactly these fields. (5) `AttemptJournal` had 24 tests and zero non-test callers, so PLAN Sec 5.4's `unknown_after_start` -- the record of work started and never measured -- could not be produced by any real run. (6) Resume validated the run's CONDITIONS but nothing about the result, so an endpoint table could be edited freely, a `failed` could be flipped to `ok`, and a genuinely failed shard was frozen as failed forever. (7) The cohort payload wrote every endpoint at depth 0, dropped the deepest rung's descendant pool and every projected state, and left the policy, A2 and admission columns null. (8) A run declaring no inputs was signed with a `no_declared_inputs` sentinel and exited 0.
- artifacts:
  - `inverse_folding/reference_flow/fusion_v2/` (12 torch-free contract modules), `inverse_folding/reference_flow/fusion_v2_runtime/` (10 tensor-side modules)
  - `scripts/rf_fusion_model_factory.py`, `scripts/rf_fusion_v2_artifacts.py`, `scripts/rf_fusion_v2_resume.py`, `scripts/rf_fusion_v2_preflight.py`, `scripts/rf_fusion_v2_cohort.py`, `scripts/run_rf_fusion_v2.py`
  - `scripts/rf_fusion_v1_artifacts.py` (ledger writer widened to accept mappings; purely additive)
  - `scripts/rho_maturity_scan.py --mode step` (the `B(r)` band calibration this consumes)
  - `doc/SCRIPTS.md` (items 13-18), `PROGRESS.md` (open-gaps table)
- evidence: 1121 V2 tests pass; repo-wide 3611 passed with the failing set BYTE-IDENTICAL to clean HEAD (47 entries, all pre-existing missing-dependency or missing-fixture). 348 V1/v0 regression tests pass and `inverse_folding/reference_flow/sampler.py` is byte-identical to HEAD. 27 guards were mutation-tested here in isolation (neutered one at a time, suite run, file restored and hash-verified): all 27 KILLED, no residue. Two delegated passes report a further 31 by the same protocol; those are re-run as part of the suites above but their individual mutation runs were not repeated here. The substrate and cap defects were reproduced by direct execution before being fixed (654-vs-374 DFE; 4 x 250000 against a 500000 cap exiting 0). The cost fix is reconciled three ways on one schedule -- `project_v2_budget` == `LadderOutcome.total_logical_dfe` == counted denoiser forward passes == 374 -- so the preflight bound, the ladder's report and the execution graph cannot drift apart; the pre-fix ladder reported 380 and the pre-fix preflight 318. An end-to-end fake-oracle shard emits endpoint depths {0,1,2} (was {0}), both live and projected partial states, 2 A2-view rows, populated policy and admission columns, and 16 ledger rows over 6 phases whose committed `logical_dfe` sums to the same 374.
- impact:
  - scope: `fusion_v2` and `fusion_v2_runtime` only, plus the six V2 scripts and one additive widening of the shared V1 ledger writer. `run_one_cycle` and `run_depth_ladder` gained two REQUIRED keyword arguments (`feedback_enabled`, `cost_meter`); no V1 or v0 path calls either. V1 artifact output is unchanged.
  - risk: medium
  - confidence: 0.90
- status: done
- next_action: None in code. The launcher and Canary commands are outside this PLAN revision (Sec 5.5) and belong to the runbook; before writing it, resolve the `explicit_probe` gap recorded in `PROGRESS.md`.
- refs:
  - PLAN_RF_REFINE_FUSION_V2.md Sec 2.4/2.5 (kernel/policy boundary), Sec 3.4 (cost model), Sec 4.3 (A2 matching), Sec 4.5 (typed stopping), Sec 5.2-5.5 (provenance, evidence objects, resume, reuse-first driver)
  - Still NOT fixed, recorded in `PROGRESS.md`: PLAN Sec 2.5 authorizes `explicit_probe` for "the first state-transition diagnostic", but `ExplicitProbePolicy` takes PREDECLARED support sets while the kernel requires `reopen` to name only source-RESOLVED positions -- and which positions are resolved at `c_d` is stochastic. Measured on the real kernel: `null_invalid_policy_result`. A canary configured this way would pay a prefix, K lookaheads, a Head batch and K refolds per cycle and measure only typed nulls. The rule needed to fix it (reopen cardinality) is one of the four quantities Sec 2.5 forbids authorizing "until it is explicitly frozen in config/runbook provenance", so it is deferred to the runbook by decision rather than patched here.
  - PLAN Sec 4.5 types `no novel descendant` but does not define novelty; the implementation chooses the archive's own `sequence_equivalence_key` and says so in the code. Before this, a degenerate run reached the depth cap over ONE unique sequence and reported `depth_cap`.
  - `active_population_width > 1` stays refused at config parse: Interface Map OQ7 leaves open whether a forked family inherits its parent's `reference_binding_id` or opens its own, and the ladder advances exactly one lineage per cycle (PLAN Sec 4.4).
  - `reward_ordered` / `policy_source_off` stay interface-only: PLAN Sec 8.4 sequences the `FeedbackSupportPolicySpec` freeze AFTER positive source transmission, and a placeholder would yield a number that looks like evidence.
  - Local fake-oracle success validates WIRING ONLY. Real feedback transmission, structure, timing and calibration remain unmeasured; production `D>1` stays launch-disabled and nothing here is `CANARY-READY`.

### L0144
- timestamp: 2026-08-06T21:30:00-04:00
- type: FEAT
- module: RF/FUSION_V2
- trigger: Runbook §3-§6 described how to launch the state-transition Canary but nothing produced a resolved cell, and `--dry-run` -- the only gate before a GPU allocation -- loads no model and therefore returns BEFORE `build_v2_oracles`, which is where every launch-blocking fault turned out to live. Runbook §8 had already recorded the exposure ("Nothing in V2 has run on a cluster. Every local suite uses fake oracles and validates WIRING only."); this is that exposure cashed.
- change_summary: Added the Canary launch surface -- a per-cell config materializer that also emits the driver's argument vectors, a GPU-free assembly preflight that runs the real contract chain, a one-cell SLURM launcher, and a §6 reader that refuses to compute a Head contrast -- fixed the four runtime defects the first real executions exposed, and executed the four-cell Canary.
- rationale: Each defect was invisible to 1121 local V2 tests for the same structural reason: the fakes define whatever production asks for, so a real-vs-fake divergence can only surface on a real object. (1) `build_v2_oracles` never passed `fixed_token_policy`, which `build_model_factory` defaults to the literal `"unconstrained"` -- the anchors were always enforced, but the conditioning row every artifact carries named the wrong policy on the anchored protein, and the two Canary strata, which differ in exactly this, declared the same one. (2) `alphabet=model.alphabet`, which `PreparedModel` calls `id_to_aa`; the identical rename had already cost the hotspot calibrator one allocation. (3) `OracleSeams.resolved()` handed back the REFUSAL sentinel `_default_gpu_clock`, and the driver injects no seams, so the production stack had no GPU instrument at all and every launch died in `CostMeter` on the first attempt -- a guard no production path can satisfy does not protect the measurement, it prevents it. (4) `run_one_cycle` calls `support_policy(source, endpoint, coordinates)` positionally while the `FeedbackSupportPolicy` Protocol declared only `decide(*, ...)`, so an implementation could satisfy the declared interface and be unusable by the only thing that consumes it; found at a real projection, after the structure gate had already admitted an endpoint. Three config-role bindings the runbook could not have known until a cell ran are recorded in §3: `head_config`/`head_checkpoint` must be FROZEN (the reference's evaluator identity is built from them while every endpoint's comes from the scorer, and `bind_head_scores` compares the two), an unconstrained cell must still DECLARE its lack of constraints (`rho_maturity_scan`'s typed-absence digest, the same number `5ZHV_B`'s own `B(r)` already carries), and `content[complete_reference_sequence]` is the cell's own `.seq` rather than the manifest.
- artifacts:
  - `scripts/materialize_v2_canary_config.py`, `scripts/preflight_v2_canary_assembly.py`, `scripts/submit_rf_fusion_v2_canary.slurm`, `scripts/analysis/read_v2_canary.py`
  - `scripts/rf_fusion_v2_oracles.py` (`fixed_token_policy_label`, `reserved_gpu_seconds_clock`, `id_to_aa`), `inverse_folding/reference_flow/fusion_v2/policy.py` (`__call__` on the Protocol and both implementations)
  - `doc/RF_Fusion_v2_Cluster_Runbook.md` (§3 notes 3/6/7/8, §4 second gate, §5 launcher, §5.1 execution record), `doc/SCRIPTS.md` (22-25)
  - `<WORK>/v2_canary/resolved_configs/{5zhv_r30,5zhv_r40,q00511_r30,q00511_r40}.{yaml,args.sh}`, `<WORK>/v2_canary/strata.json`, `<WORK>/v2_canary/canary_read_v1.json`, `<RUN>/inverse_folding/v2_canary/<CELL>/`
- evidence: Four cells, jobs `12094327-30` on rtx6000 under `immune-design-blackwell`, ~2 min and 8.6-35.9 GPU-s each. **3/4 committed a state transition**; both proteins transitioned, which is §6's own per-protein condition. Support partitions sum to the editable domain exactly -- 102 for `5ZHV_B`, **278 = 302 - 24** for `Q00511`, so hard anchors are in no support class by measurement and not only by construction. The coupled identity `u_proj = u_src - a + b_new` closes to the digit in every committed cell and every projected state lands INSIDE its own `B(r)` envelope on both axes (`5zhv_r40` 55 in [53,70], rho 0.461 in [0.315,0.480]; `q00511_r30` 138-1+50 = 187 in [186,208], rho 0.327 in [0.253,0.330]; `q00511_r40` 138-1+21 = 158 in [157,179], rho 0.432 in [0.357,0.435]) -- so the band actually pinned the reopen cardinality rather than merely being declared. Anchors: 24 x 8 endpoints against WT, **0 violations**, both `Q00511` cells. Assimilation passes as four separate checks (injected positions `pending_assimilation` with a NULL score at projection, `assimilated` with a finite one after the first propagation, every protection grant expiring at `c_{d+1}` exactly, and no `evidence_logprob` appearing as an active sampler score). Ledger `physical_cost_complete` with all six phases and no breached cap. Local suites: 1016 fusion_v2 + 39 oracle tests; the three new guards are mutation-verified (restore each bug, the matching test fails with the exact cluster error).
- impact:
  - scope: four new scripts, two runtime fixes in the V2 oracle stack, one interface addition in the V2 policy contract. `__call__` is additive -- every existing caller passes a bare function and is unaffected. No V1/v0 path touches any of it. The `fixed_token_policy` fix CHANGES the conditioning digest of any anchored V2 run relative to before, which is the point.
  - risk: medium
  - confidence: 0.90
- status: done -- verdict frozen `WIRING_PASS_WITH_POPULATION_MISMATCH_DIAGNOSTIC`. The Canary's one question was whether the mechanism EXECUTES; both proteins produced a legal transition with every invariant closed. `5zhv_r30` is retained as a real structure-gate / sibling-correlation diagnostic and will NOT be rerun -- re-seeding for a fourth green cell is post-outcome rescue and adds no wiring evidence.
- next_action: The population mismatch belongs to the NEXT experiment's population definition, not to whether the Canary passed. Owner's order: (1) recompute `N_H^whole` on the existing bundles; (2) land failed structure evidence in a `structure_evaluations` artifact; (3) resumed-null calibration with the SOURCE PREFIX as the sampling unit -- 32 independent `c=50` prefixes x 2 feedback-off completions per protein; (4) freeze mechanism-stage gates from it (native-relative `Delta_anchor` at Q0.90 higher, `scTM_min` at Q0.10 lower, mechanism-operability ONLY); (5) design the powered one-cycle cohort; (6) `D>1` stays closed. `5zhv_r30` admitted no endpoint, and the reason is a population mismatch recorded in §5.1: the structure gate was calibrated on full de novo trajectories (scTM median 0.8528, 50% pass) but enforces on completions RESUMED from a captured source state (median ~0.798, 1/8 pass; descendants ~0.712). Changing `n_lookaheads`, the band or the gate is not authorized here.
- refs:
  - The same population question is OPEN on the Head axis and is not answered by this run: the hotspot threshold was calibrated on the full-trajectory population and enforces on the resumed one, and the artifact records `head_global_risk` rather than `N_H^whole` for Canary endpoints. §2.1 already answered this question once for the structure axis; it has not been asked for the Head axis.
...
### L0146
- timestamp: 2026-08-07T02:10:00-04:00
- type: FEAT
- module: RF/FUSION_V2
- trigger: Runbook §7's mechanism cohort needed an executable readout. The Canary showed the feedback machinery EXECUTES and `source_dependence_verdict` shows the kernel is not blind to what it is handed, but both stop at the PROJECTED state; whether the injected identity survives propagation to a completed descendant is the actual scientific claim of V2 and nothing measured it.
- change_summary: Descendant-level transmission statistic, its execution path, and its reader. Normalized Hamming between a matched pair of complete descendants on the FREE DOMAIN -- editable positions still unresolved in the projection at `r_d` -- carried by a new `mechanism_contrasts` bundle table, produced by `run_v2_mechanism_shard` behind `--mechanism-prefixes N`, and read by `scripts/analysis/read_v2_mechanism.py`.
- rationale: The domain IS the design. `inject_from_source_feedback` writes the SOURCE's own token and `carry_from_source` copies source rows verbatim, so under `source_shuffle` the permuted bytes reach both arms' projections by construction; a whole-editable Hamming then reads 0.33-0.46 on the executed Canary coordinates for a kernel that transmits nothing, and a gate at 0.02 on that domain is not a gate. Stating the domain as ONE predicate over the projected state -- rather than as a list of exclusions -- makes hard anchors, the endpoint-written position and both source-carrying classes fall out automatically and cannot drift from the kernel. Execution reuses `run_mechanism_views`, which already pays for the source prefix and its lookahead pool once and forks every arm off that single realized pool; the shard shares the driver's config, signature, oracles, fragments, resume, ledger and bundle with `run_v2_shard` rather than duplicating them.
- artifacts:
  - `scripts/rf_fusion_v2_artifacts.py` (`mechanism_contrasts` schema, `free_domain`, `mechanism_contrast_rows`; `feedback_events` join key gains `arm_slot`/`treatment_identity`)
  - `scripts/rf_fusion_v2_cohort.py` (`run_v2_mechanism_shard`), `scripts/run_rf_fusion_v2.py` (`--mechanism-prefixes`, `select_runner`)
  - `scripts/analysis/read_v2_mechanism.py`, `scripts/materialize_v2_canary_config.py` (`--campaign-id`), `scripts/submit_rf_fusion_v2_canary.slurm` (`MECHANISM_PREFIXES`)
  - `inverse_folding/reference_flow/fusion_v2_runtime/paired.py` (per-arm `origin_transition_id`)
  - `doc/RF_Fusion_v2_Cluster_Runbook.md` §7 (rewritten), `doc/SCRIPTS.md` (22, 23, 18, 28)
  - `<WORK>/v2_canary/resolved_configs/{5zhv,q00511}_mech.{yaml,args.sh}`, `<RUN>/inverse_folding/v2_mechanism/`
- evidence: One-prefix smoke on `5ZHV_B` at `r=40` (job `12098622`, EXIT=0, 2:00, MaxRSS 43.5 GB) validates the statistic on real oracles. `endpoint_change`: `n_input_diff = 1` -- the intervention is exactly one amino acid, as `write_from_endpoint=(write,)` requires -- propagating to 3/1/1/4 residues of a 53-position free domain. `source_shuffle`: 41 conditioning tokens permuted, 19-22 propagated. **The domain model is confirmed by measurement, not only by argument**: for `endpoint_change` `n_diff_editable` equals `n_diff_free` on every fork (3=3, 1=1, 1=1, 4=4), so every difference the endpoint channel produced lies inside the free domain; for `source_shuffle` the two differ by ~41 -- exactly `n_input_diff` -- so the whole-editable reading is 0.60 where the free-domain reading is 0.39, and a 0.036 gate on the editable domain would pass the positive control on pure construction. `structure_evaluations` is populated by a cluster shard for the first time (20 evaluated, 16 feasible). Ledger: 35 attempts, 35 distinct events, `n_retries = 0`, `root_capture = 1` and `screen = 1` -- the shared pool is paid once. Tests: 1019 fusion_v2 + 258 script/config; both cluster guards mutation-verified.
- impact:
  - scope: `mechanism_contrasts` is additive and empty for ladder runs. The `feedback_events` key change is a no-op for unpaired runs (`arm_slot` constant). The per-arm `origin_transition_id` changes what a paired run writes into position provenance and into `feedback_events.transition_id`; no ladder path is affected.
  - risk: medium
  - confidence: 0.88
- status: in_progress -- batch 1 (16 independent prefixes per protein, jobs `12098724`/`12098725`) submitted.
- next_action: Read batch 1 with `read_v2_mechanism.py` (no `--confirmatory`), take `sigma_d` from `endpoint_change`, derive `n_total`, run batch 2 to completion and analyse both batches together. This is an INTERNAL pilot: batch 1 is not discarded.
- refs:
  - Two cluster-only defects, both invisible to 1049 local tests for one structural reason -- the paired-executor suite passes every argument explicitly, so a real-vs-fake divergence can only surface on a real object. (1) `run_one_cycle` takes 31 required keyword-only arguments; the factory supplies most and the LADDER supplies the rest at its own call site, so a second caller that only forwards `cycle_kwargs` is missing exactly the ladder's extras (job `12098131`, `origin_transition_id`). Now guarded by an AST contract test that derives every term from production source. (2) One transition id shared across matched arms made 13 arms-doing-their-own-work reads look like retries against a 2-retry cap (job `12098305`). The cap was NOT raised: the retry guard exists so "a retry cannot inflate the number a matched-compute claim is compared on", and a shared id blinds exactly that. Arms are two transitions, not one attempted twice.
  - Also measured only by running it: a one-prefix run peaks 39-43 GB where a Canary cell peaks 27.5-30.4 GB. The header stays sized for a Canary cell; the mechanism cohort passes `--mem=64G`.
  - `delta = 0.036` on the free domain is the pre-registered effect and is NOT read off batch 1. Batch 1's only decision output is `sigma_d`.

### L0147
- timestamp: 2026-08-07T04:05:00-04:00
- type: EXPERIMENT
- module: RF/FUSION_V2
- trigger: Runbook §7's mechanism cohort -- the only stage whose result can open `D>1`, and the first V2 experiment to ask whether the projected identity REACHES the completed descendant rather than whether the machinery executes.
- change_summary: Executed as a two-batch internal pilot and read under the pre-registered gate. **`transmission_demonstrated`** on the primary; the structure secondary is **inconclusive by construction**, so `D>1` stays shut.
- rationale: `write_from_endpoint` is a single position by construction, so the entire endpoint channel at `D=1` is ONE amino acid. The experiment therefore asks whether one conditioning token measurably steers the completion. The statistic is normalized Hamming between matched complete descendants on the FREE DOMAIN -- editable positions still unresolved in the projection at `r_d` -- because `inject_from_source_feedback` writes the source's own token and `carry_from_source` copies source rows verbatim, so a whole-editable denominator scores the construction rather than the mechanism.
- artifacts:
  - `<RUN>/inverse_folding/v2_mechanism/{5zhv_mech,q00511_mech}/`, `<RUN>/inverse_folding/v2_mechanism_b2/{5zhv_mech,q00511_mech}/`
  - `<RUN>/inverse_folding/v2_mechanism/mechanism_confirmatory.json`, `batch1_read.json`
  - `<WORK>/v2_canary/resolved_configs/{5zhv,q00511}_mech.{yaml,args.sh}`
  - `doc/RF_Fusion_v2_Cluster_Runbook.md` §7.5 (realized sizing), §7.8 (result), §7.9 (secondary), §7.10 (reconciled)
- evidence: Batch 1 jobs `12098724`/`12098725` (prefixes 0-15), batch 2 jobs `12099604`/`12099605` (prefixes 16-55), analysed as ONE sample. `endpoint_change`: `5ZHV_B` 38/56 scored, mean 0.06233, **CI95 lower 0.04903 > delta 0.036**; `Q00511` 39/56 scored, mean 0.09355, **CI95 lower 0.07783**. `source_shuffle` positive control: 0.31985 (CI lower 0.30566) and 0.22182 (CI lower 0.21349). All four components of the conjunctive gate pass. Realized `sigma_d_max = 0.0485` gives an unclipped 15, so 38/39 scored prefixes exceed the 32-prefix floor -- not underpowered by its own rule. In residues: **3.4 of 54.3 (`5ZHV_B`) and 14.8 of 158.2 (`Q00511`) from a ONE-token change**; in batch 1 not one of the 22 scored prefixes across both proteins gave an exact zero. Structure secondary: depth-0 pool 94.2% / 90.2% feasible vs descendants 67.3% / 64.6%.
- impact:
  - scope: no code behaviour change; this entry records an experiment. The runbook gains the realized result and the reconciled `D>1` precondition.
  - risk: low
  - confidence: 0.90
- status: done -- primary `transmission_demonstrated` AT `r=40`; secondary inconclusive; `D>1` remains launch-disabled.
- scope: **one coordinate.** `r=40`, `c_source=50`, `c_next=60`, `D=1`, one policy, write cardinality 1, 4 lookaheads. The only factor with two levels is the protein. `r` is very unlikely to be inert -- a deeper re-entry leaves more forward steps after the injection, and the band pins more reopens there (50 at `r=30` vs 21 at `r=40` on `Q00511`, measured on the Canary) -- so BOTH the transmission effect and the structure cost plausibly move with it. The reported 3.4 / 14.8 residues are a point on an unmeasured curve, not a property of V2, and a `r=40` result licenses nothing at `r=30`.
- next_action: The missing arm. §7.6's three-way structure reading CANNOT be completed on this design, because every descendant-producing arm is feedback-on -- a feedback-disabled cycle stops after the A2 view and produces no descendant at all. So "projection damages structure" and "resumed propagation to `c_next` loses structure" are not separated, and the ~26-point drop cannot be attributed. Separating them needs a descendant produced with NO projection, matched on source, fork seeds and horizon. Until that exists, §7.10's precondition is unmet on the structure side regardless of the primary.
- refs:
  - The domain choice is now measured rather than argued. For `endpoint_change`, `n_diff_editable` equals `n_diff_free` fork by fork -- every difference the endpoint channel produced lies inside the free domain. For `source_shuffle` the whole-editable reading is 0.594/0.529 against 0.320/0.222 on the free domain, the gap being exactly the permuted source bytes that reach both projections by construction. **A `delta = 0.036` gate on the editable denominator would have passed the positive control with no transmission at all.**
  - Two batches ran under two code revisions (`af83803f0a38`, `2844d0edad3d`); the only run-path difference is the loop bound selecting WHICH prefix indices run. Established by evidence, not by reading the diff: prefix 0 replayed under the later revision (job `12099520`) is byte-identical to batch 1's prefix 0 in every contrast column including descendant sequence digests.
  - Batch 2's size was fixed BEFORE it ran, from batch 1's measured contrastable rate: the naive 21/0.688 ~= 31 prefixes reaches the 32-scored target only 63% of the time, 40 reaches it 99%. `n_pairs` is a power floor, not a cap; every scored prefix entered the analysis.
...
- refs:
  ...
  - **Both stage controls landed and they sharpen the reading further: against a STAGE-MATCHED comparator the per-cycle cost is near zero, not 4 points.** Natural completions captured at `c=60` under the mechanism gate score 93.8% (`5ZHV_B`) and 85.9% (`Q00511`); the V2 descendant scores 90.0% / 85.9% at `r=40` and 93.3% / 85.5% at `r=30`. `Q00511` matches its control exactly. The 4 points reported above compared a step-60 descendant against a step-50 pool, so most of it was the stage difference it was meant to exclude.
  - Bounding caveat on every absolute figure: the two independent estimates of "natural completions from `c=50`" disagree by 9 points on `Q00511` (90.2% mechanism pool vs 81.2% resumed null, different seed namespaces). Cross-run absolute comparisons carry several points of sampling noise; only the within-run paired `r` comparison is tight.

### L0149
- timestamp: 2026-08-07T13:40:00-04:00
- type: FEATURE
- module: RF/FUSION_V2
- trigger: Runbook §7.11 read one-cycle reward ordering as `not_demonstrated`. The diagnosed cause is not that feedback cannot carry value: the DONOR was chosen by exact complete Head rank while the written and reopened positions were chosen by source mask geometry, so the applied dose was never ordered by anything the Head said about that position. PLAN task V2F5A replaces the support law and changes nothing else.
- change_summary: `HeadDirectedCappedPolicy` + its matched `SourceGeometryControlPolicy` land, together with the lineage reward incumbent, same-scale aligned-window evidence, the frozen-Head leave-one-out counterfactual, the integer band centre and the exact reopen equation. The projection kernel, the frozen substrate, the propagation horizon, the band solver and the descendant seed law are untouched.
- rationale: The three guarantees PLAN §2.5/A.2 separates are now separately enforced. **Donor direction**: feedback may originate only from a definitively feasible endpoint with `R_H(Y*) < R_H(I_d) - eps_R` against the lineage's own incumbent, else a typed `stall_no_better_donor`. **Applied-dose direction**: a write candidate must be editable, non-anchor, source-UNRESOLVED, differ from the incumbent and lie in a raw aligned Head window improved in the donor; it is then ranked by `a_i = R_H(Y*_{i<-I_i}) - R_H(Y*)` from the frozen Head, and only positive `a_i` above the calibrated floor may be written -- zero of them is `stall_no_positive_local_write`, never a fallback token. **Archive non-regression** is unchanged. The one-token cardinality is retired: `m_d = min(n_positive, ceil(f*N_editable), m_band_compatible)`, a cap and not a quota, with `m_reopen = u_target - u_src + m_d` solved exactly against a content-bound integer band centre so the projected state LANDS on the declared target rather than merely inside `B(r_d)`.
- artifacts:
  - `inverse_folding/reference_flow/fusion_v2/{policy,evidence,reward,schedule,config}.py`
  - `inverse_folding/reference_flow/fusion_v2_runtime/{cycle,paired,contribution}.py`
  - `scripts/{rf_fusion_v2_oracles,rf_fusion_v2_artifacts,rf_fusion_v2_cohort,rf_fusion_v2_preflight}.py`
  - `scripts/analysis/replay_v2_head_directed_policy.py` (registered, `doc/SCRIPTS.md` 29)
  - `tests/inverse_folding/test_fusion_v2_head_directed_policy.py`
- evidence: `pytest tests/inverse_folding/test_fusion_v2_*.py tests/scripts/test_rf_fusion_v2_*.py -q` -> 1250 passed (52 new). `pytest tests/inverse_folding/test_reference_flow_sampler_continuation.py tests/inverse_folding/test_reference_flow_fusion_*.py tests/scripts/test_rf_fusion_v1_*.py tests/scripts/test_run_rf_fusion_v1_entry.py tests/scripts/test_run_rf_refine_fusion.py -q` -> 666 passed, so V1 replay and v0 complete-state selection are intact. The end-to-end tests run a real cycle under the new policy and a matched `support_law` contrast: both arms take one donor, one source, one realized pool, identical descendant seeds and identical `(m_write, m_reopen)`, and the artifact proves which arm was Head-directed (`head_evidence_consulted`).
- impact:
  - scope: additive. No existing policy, config or bundle changes behaviour -- `projection.head_directed` is REQUIRED for the new policy and REFUSED for any other, so a frozen S7 config still loads and still runs `state_derived_probe`. `run_one_cycle` hands a `PolicyRuntime` only to a policy that advertises `consumes_runtime`, so every other policy keeps the three-argument call shape. `feedback_events` gains 25 evidence columns, null for policies that emit none.
  - risk: medium -- this is the first V2 policy that calls the frozen Head WHILE deciding. That batch is journaled before it runs (PLAN §5.4), so it spends against `max_head_calls` visibly, but its size is one call per legal write candidate and is NOT in the budget projection; the offline replay measures the real distribution before anything launches.
  - confidence: 0.75
- status: done (code) -- NOT qualified. No offline replay has been run on the S7 bundles and no cluster comparison has been launched.
- next_action: (1) measure `eps_R` and the local contribution tolerance from the frozen Head's repeatability / numeric floor and write them as typed `PolicyCalibrationArtifact`s -- the loader refuses an invented value, so nothing can start without them; (2) run `replay_v2_head_directed_policy.py` on the S7 mechanism bundles for the §8.4 coverage gate; (3) only then freeze the one-cycle matched comparison.
- refs:
  - **The depth-0 incumbent is an OPEN decision and is declared, not defaulted.** PLAN §3.1 leaves the `I_0` binding open, so `config.projection.head_directed.lineage_incumbent_depth0_rule` is required with a closed vocabulary. `cumulative_safety_reference` (`I_0 = ybar`) is implemented and is frozen before any depth-0 endpoint is scored by construction; `predeclared_external_design` is declarable and refused by the oracle factory until it has its own content-bound input role, because an unsigned sequence may not decide which donors open feedback.
  - Cross-sequence local evidence is built ONLY from raw aligned window `z`, never from `residue_hotspot`: a per-sequence summary was centered inside that sequence, so subtracting two of them across sequences reports the sign of the transform rather than of the design. Enforced by an AST test asserting `fusion_v2/evidence.py` never reads the attribute.
  - The same AST test was strengthened from PLAN §2.5's original single-module check to the whole import closure of `fusion_v2/policy.py`: V2F5A adds two modules to that closure, and a rule that only checked one file could be satisfied while still handing the policy a path to the run config.
...
### L0165
- timestamp: 2026-08-25T07:10:00-04:00
- type: FEATURE
- module: RF/FUSION_V2
- trigger: Fusion V2 steers on one frozen epitope Head, so a design that is safe under HLA-DRB1*07:01 has no measured status under any second allele. The project needs an optional second-allele steering mode that adds a real objective rather than a post-hoc filter, without moving a single byte of the frozen single-Head substrate.
- change_summary: Optional dual-allele mode for Fusion V2. Two Heads on incomparable raw scales are mapped through frozen affine coordinates `u_a = (R_a - b_a)/s_a` and reduced by a bounded smooth worst-residual scalar `J_tau`, with exactly ONE human-chosen number -- the credit `c_u = tau*log2 = 0.10` normalized risk units, hence `tau = 0.14426950408889636`, frozen in `configs/v2_dual_smoothmax_policy_v1.json` and cross-checked against the credit on load. Arms name the objective and nothing else (`joint` reduces both, `a_only`/`b_only` order on one); both Heads are scored, bound and published in every arm, and arms are compared ACROSS runs because the arm enters no seed derivation. Decision margins are derived, never declared: `eps_donor = eps_write = 2*max_a(e_a/s_a)`, twice the 1-Lipschitz single-measurement bound because both gates compare a DIFFERENCE. There is no `eps_J`. The per-cycle candidate domain C is the overlay's own field and is NOT the legacy Head-call field; the logical Head-call budget 2C is projected by the preflight. With no overlay the Dual layer is never imported and every config digest, endpoint id, ordering and decision is unchanged, pinned to literals in `test_fusion_v2_dual_off_compatibility.py`. The calibration producer additionally supports campaign re-signing (`--resign-from`, binding the Head-B runtime from the signed artifact and refusing a contradicting `--head-b-*`), overlap-fraction carry-in (`--panel-report`), the overlap-inclusion sensitivity (`--compare-to`/`--out-comparison`, emitting `dual-overlap-sensitivity-1`) and shift recording (`--overlap-inclusion-shift`); the panel builder gained `--head-overlap exclude|include`, which measures the Head-training overlap under both policies and removes it under one.
- rationale: A second allele is only a scientific gain if it enters the objective that selects ancestry. Filtering afterwards cannot recover a design the search never proposed. Making the mode optional and structurally absent -- rather than defaulted-off -- is what keeps the frozen V2 result reproducible byte for byte while the Dual layer exists in the same tree. Deriving both margins from measured instrument drift, instead of declaring them, is what stops the gates from admitting noise; and splitting the ceiling into a candidate domain (unchanged) and a Head-call budget (doubled) is what stops a second instrument from halving the editable domain.
- artifacts:
  - `inverse_folding/reference_flow/fusion_v2/{joint_objective,dual_evidence,dual_policy,dual_config}.py`
  - `inverse_folding/reference_flow/fusion_v2_runtime/{dual_runtime,dual_selection,dual_lookahead}.py`
  - `inverse_folding/reference_flow/fusion_v2/{policy,reward}.py`
  - `inverse_folding/reference_flow/fusion_v2_runtime/{cycle,ladder,archive}.py`
  - `inverse_folding/reference_flow/configs/v2_dual_smoothmax_policy_v1.json`
  - `scripts/{calibrate_v2_dual_objective,build_dual_calibration_panel}.py`
  - `scripts/{run_rf_fusion_v2,rf_fusion_v2_cohort,rf_fusion_v2_oracles,rf_fusion_v2_artifacts,rf_fusion_v2_preflight,rf_fusion_v2_resume,materialize_v2_canary_config,preflight_v2_canary_assembly}.py`
  - `scripts/submit_rf_fusion_v2_canary.slurm`
  - `doc/{SCRIPTS.md,Dual_Allele_Steering.md,DUAL_ALLELE_DUALF0_AUDIT.md,RF_Fusion_V2_Dual_Allele_Cluster_Runbook.md}`
  - `PLAN_RF_FUSION_V2_DUAL_ALLELE.md`
- evidence: SLURM 12931661 full suite 4462 passed / 6 failed, the 6 being the pre-existing dplm_refiner tensor-shape and hardcoded-path failures unchanged from 12927433 (4445 passed); 355 of the passing tests are the Dual/joint-objective/calibrator/panel suites. Calibration MEASURED on this cluster (SLURM 12891919, 1 h 26 m on one h200) over a frozen 13,836-protein allele-neutral natural panel (SLURM 12891590): cross-allele Pearson r = 0.0962 against the 0.19 within-family estimate the domain-transfer argument was calibrated on, bootstrap SE(b_A/s_A - b_B/s_B) = 0.01847 against c_u = 0.10, normalized intercept -0.4265, measured Head-training overlap 14.12 % (A) / 15.78 % (B) with 1.66 pp asymmetry -- an overlap that had never been computed before. Re-signing the one measured calibration into the two campaign overlays takes 0.6 s and was verified through the production seam (`load_dual_overlay_file` -> `build_arm_objective`): distinct overlay digests `8d932953e545` (C=278) and `889c365c6350` (C=454), identical per-arm objective digests `5cbf477a90a9`/`8774141edebd`/`91d52880e5ed`, so the two campaigns differ in candidate domain and share one objective. Overlap-inclusion sensitivity MEASURED (panel SLURM 12931496, calibration SLURM 12931501, 1 h 32 m): the include panel is identical to the primary through `after_canonical` = 56,475 and on f_A/f_B and yields 14,937 clusters against 13,836; including every Head-training homolog moves the equal-risk line by delta_theta = -5.6885e-4, i.e. |delta_theta|/c_u = 0.0057 and about 1/32 of the panel's own bootstrap SE, with delta_log(s_A/s_B) = 3.75e-3 -- label `within_credit`. Two launchable overlays signed: `dual_overlay_m1_v1.json` (C=278, digest f70b96ffe9ad) and `dual_overlay_c1_v1.json` (C=454, digest be53f59fd493), byte-identical calibration blocks, both carrying f_A/f_B and |delta_theta|, resolving to the same three arm objectives fab070d27238/3928c36074ab/4c168fa7c156.
- impact:
  - scope: additive and opt-in. A run without `--dual-overlay` imports no Dual module and is byte-identical to the frozen V2; the legacy `max_counterfactual_head_calls_per_cycle` field is never reinterpreted.
  - risk: medium
  - confidence: 0.93
- status: implemented, tested, calibrated, and both campaign overlays signed; no generation job submitted.
- next_action: Hand `doc/RF_Fusion_V2_Dual_Allele_Cluster_Runbook.md` to an execution agent, which starts at §2.2 because P0's calibration is complete. S6's 80/100 coverage floor is a READ-TIME user decision (a healthy single-root campaign falls below it ~5 % of the time) and does not block submission.

### L0166
- timestamp: 2026-08-25T11:05:00-04:00
- type: FIX
- module: RF/FUSION_V2
- trigger: A pre-handoff adversarial review of the Dual-allele package (5 independent lenses, every finding put to a skeptic instructed to refute it) returned 14 confirmed findings and 1 refuted. Two changed published numbers, two would have wasted GPU-days, and several were introduced by L0165's own documentation.
- change_summary: (1) `equal_risk_line_stderr` resampled proteins with replacement and then re-keyed the resampled rows by their ORIGINAL protein_id, so `location_and_scale`'s protein-equal weighting collapsed a protein drawn three times back to weight one -- erasing the re-weighting a bootstrap uses to simulate sampling variability and leaving only the spread of a random ~63.2% subset. Each draw is now its own weighting unit. (2) `--resign-from` hashed `--objective-spec` into `objective_spec_digest` without ever parsing it, so a tau sweep left at that path would be adopted as the artifact's stated authority; the re-sign now refuses a spec whose tau, declared credit or law version contradicts the calibration it carries forward. (3) `print_config_payload` projected a SINGLE-Head budget for a Dual run while `assert_launch_feasible` correctly projected two, so the launch evidence an operator records understated Head spend by one allele -- and the cost-parity GO criterion is read against that printed number. (4) `resolve_dual` now refuses any overlay carrying no measured overlap-inclusion sensitivity: four structurally valid signed overlays exist in the calibration tree, only two may be launched, and nothing in the launch path told them apart. The same guard closes re-signing FROM the contaminated sensitivity artifact, because that drops the field too. (5) `--compare-overlays` rebuilds a sensitivity report from two signed artifacts with no GPU; `--recompute-equal-risk-stderr` re-derives the standard error from the signed evidence table and refuses unless that table's sha256 equals the artifact's own `calibration_artifact_digest`; `--panel-overlap-policy` lets a re-sign of the sensitivity artifact accept its own report. (6) The three CLI paths (measure, re-sign, compare) now require only what each needs, instead of forcing a re-sign to restate sixteen instrument values by hand -- which was the hazard, not the safeguard.
- rationale: The bootstrap defect is the important one: the coordinates were never affected, because each protein appears exactly once in the full-panel estimate and protein-equal weighting is exactly right there. Only the resampled statistic was wrong, and only in the direction that flatters it. The rest share a shape -- an artifact or a projection that is structurally valid and quietly disagrees with the instrument, the evidence or the budget it claims. A margin derived from measured drift cannot protect against a value nothing ever compared.
- artifacts:
  - `scripts/calibrate_v2_dual_objective.py`
  - `scripts/rf_fusion_v2_preflight.py`
  - `scripts/run_rf_fusion_v2.py`
  - `tests/scripts/test_calibrate_v2_dual_objective.py`
  - `tests/scripts/test_preflight_v2_canary_assembly.py`
  - `tests/scripts/test_run_rf_fusion_v2.py`
  - `doc/{DUAL_ALLELE_DUALF0_AUDIT.md,Dual_Allele_Steering.md,RF_Fusion_V2_Dual_Allele_Cluster_Runbook.md}`
  - `PLAN_RF_FUSION_V2_DUAL_ALLELE.md`, `PROGRESS.md`
- evidence: Every fix RED-first. Dual/joint-objective/calibrator/panel/driver suites 365 passed. The corrected bootstrap was re-derived on CPU from the signed evidence tables (SLURM 12946466, ~7 min/panel, no GPU): equal_risk_line_stderr 0.018471 -> **0.024044** on the primary panel (SE/c_u 0.185 -> **0.240**) and 0.017983 -> 0.023444 on the sensitivity panel. Direction verified against a hand-reconstructed collapsed estimator on the real 13,836-protein table, and the distinct-protein retention measured at 8,754/8,757/8,795 of 13,836 across three draws (~63.3%, matching 1-1/e). All four overlays re-signed: `dual_overlay_v1` 82fbb45ea1e5, `dual_overlay_overlap_included_v1` ce863e510392, **`dual_overlay_m1_v1` 7b8f66ee14ea (C=278)**, **`dual_overlay_c1_v1` fb428357287e (C=454)**; delta_theta unchanged at -5.6885e-4, `within_credit`. The launch gate was exercised on the real files: m1/c1 LAUNCHABLE, primary record and sensitivity calibration REFUSED. Separately measured (SLURM 12946592), the calibration environment differs from the run environment: 200 panel sequences scored under `immune-design` (torch 2.5.1+cu121) and `immune-design-blackwell` (2.7.1+cu128) agree bit-for-bit on only 42/200 (A) and 40/200 (B), mean |delta R| 2.97e-4 / 1.89e-4, max 3.30e-3 / 1.60e-3 -- 200x to 2200x the within-environment drift e_a ~ 1.5e-6 the decision margins are derived from. The load-bearing quantity survives: the paired shift in the equal-risk line is delta_theta_env = +1.45e-4, 0.15% of the credit band and 0.26x the panel sensitivity already accepted, so the coordinates transfer and no re-measurement is required. CONDA_ENV is now pinned explicitly at the launch block.
- impact:
  - scope: the Dual calibration producer, the Dual launch gate and the printed budget projection. No coordinate, no objective, no seed law and no single-Head behaviour changed; `equal_risk_line_stderr` is reported and gates nothing (J.7 D3 retired the only requirement that read it).
  - risk: low
  - confidence: 0.95
