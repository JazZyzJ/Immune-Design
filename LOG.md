...
- type: PERF
- module: TETRAMER_GATE
- trigger: The standalone Protenix tetramer benchmark repeatedly launched one process/model load per design. The completed 1,612-design H200 run spent 84.69 GPU-h for 53.18 h of model forward time, leaving about 28.1 GPU-h in repeated startup and per-process overhead; low observed GPU utilization also risked cluster efficiency warnings. Scope was explicitly limited to Protenix tetramers on ailab H200 and PLI H100.
- change_summary: `submit_packed_pred.slurm` is now the canonical persistent-model runner: each small-array task stages its incomplete strided JSON subset and calls upstream directory-mode `protenix-run pred` once. It preflights local MSA paths, preserves per-JSON seed reset, validates every requested CIF/summary/full-data output, normalizes to the existing nested evaluator layout, handles partial direct/legacy resume safely, rejects undersized GPUs, and emits 5-second GPU telemetry. `submit_tetramer_predict.slurm` delegates to it, so a small compatibility array also amortizes model loading. H200 and explicit PLI H100 submission recipes are documented.
- rationale: Upstream Protenix v2 already supports multiple JSONs with one shared runner and calls `seed_everything()` inside each JSON inference. Reusing that supported path removes repeated 464M checkpoint/CUDA initialization without batching incompatible variable-size tetramers or changing model flags.
- evidence: 20 targeted tests pass across the packed runner, Protenix JSON builder/runner, and tetramer evaluator; `bash -n`, `git diff --check`, and H200/H100 `sbatch --test-only` pass. On the same eight representative inputs, legacy H200 process wall summed to 1,542.9 s versus persistent H200 1,224 s (1.26x throughput, 20.7% less wall time) despite the validation H200's model forwards being about 9% slower than the historical nodes. H200 telemetry: mean utilization 79.9%, 19.2% of samples below 20%, peak 68,423/143,771 MiB. PLI H100 completed in 1,357 s with mean utilization 81.6%, 18.0% below 20%, and peak 68,423/81,559 MiB. Both runs were 8/8 successful with one checkpoint load, 40 CIF + 40 summary + 40 full-data files, evaluator-visible 5 samples/design, and `skip=8 fail=0` resume. All eight historical/new feature cardinalities (`N_asym/N_token/N_atom/N_msa`) matched exactly. Protenix defaults to `deterministic=false`, so byte-identical coordinates are not claimed; H200/H100 cross-run variation confirms this is an existing model property rather than a persistent-runner contract.
- impact:
  - scope: Protenix standalone tetramer prediction launchers only; ESMFold2, monomer/refold, AF3, and Fusion runtime paths are unchanged.
  - expected_scale: The 1,612-design/22-task historical decomposition projects about 57 GPU-h at equal forward/output workload (about 1.49x throughput) because only 22 model loads remain instead of 1,612.
  - risk: low
  - confidence: 0.96
- status: done
- refs:
  - `scripts/submit_packed_pred.slurm`, `scripts/submit_tetramer_predict.slurm`
  - `tests/scripts/test_submit_packed_pred.py`, `doc/SCRIPTS.md`, `PROTOCOL/tetramer_prediction_gate.md`
  - validation jobs `11485604` (H200) and `11485603` (H100)
