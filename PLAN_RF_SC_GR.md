...
> **Superseded as a standalone experiment (2026-06-26).** The amplify mechanism (`g_max>1`) and the `pressure_source` runtime seam are **implemented** (LOG L0119/L0118) and **reused as-is**, but the standalone SC2 amplify *pilot run* is replaced by the closed-loop position-dependent **allocation** experiment in `PLAN_PLANNER_SC_GR.md`, where amplify is folded in as the **Budget** layer (swept `g_max ∈ {1.5, 2.0, 2.5}` across the `flat` / `v_target` / `v_target_x_alloc` arms) rather than swept on its own. The entry-gate analysis below still stands (it is MET); only the standalone run is retired. See `doc/Self-Cond_GR.md` §8.
