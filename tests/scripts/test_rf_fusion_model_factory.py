...
def test_the_v1_oracle_module_no_longer_builds_its_own_task_sampler_or_backbone_cache():
    """The structural half of "V1 delegates to it".

    A factory that V2 used while V1 kept its own copy would be two implementations wearing one
    name -- the exact drift PLAN V2F7 asks to prevent.  Local tests cannot run the real oracles, so
    this is asserted on the source: the calls that build these objects must appear once, in the
    factory, and not a second time in the V1 module.
    """
    import inspect

    from scripts import rf_fusion_v1_oracles

    source = inspect.getsource(rf_fusion_v1_oracles)
    for fragment in ("load_if_task(", "PositionDependentDFMSampler(", "prepare_backbone(",
                     "make_dplm_denoiser(", "build_dplm_denoiser_context("):
        assert fragment not in source, (
            f"{fragment!r} is still constructed inside rf_fusion_v1_oracles; the factory is "
            "supposed to be the single implementation"
        )
    assert "build_model_factory(" in source, "V1 does not delegate to the factory at all"
