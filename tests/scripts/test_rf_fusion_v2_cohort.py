...
def test_explicit_root_is_bound_through_factory_payload_and_journal(tmp_path):
    ...
    expected_seed = V2SeedContext(
        ...
        protein_id=PROTEIN,
    ).depth0_root_seed(checkpoint_step=C0, root_index=root_index)
    signature = _signature(
        config, production_depth_authorized=True,
        root_index=root_index, root_seed=expected_seed)
    seen = []

    def factory(*, protein_id, config, inputs, root_index):
        del protein_id, config, inputs
        seen.append(root_index)
        return {
            "cycle_kwargs": _cycle_kwargs(lineage=F.lineage(
                root_id=f"{PROTEIN}:v2:d0:r{root_index}",
                family_id=f"fam{root_index}")),
            "gpu_clock": lambda: 0.0,
        }

    status, payload = run_v2_shard(
        protein_id=PROTEIN, config=config, signature=signature, out_dir=tmp_path,
        oracles_factory=factory, production_depth_authorized=True,
        root_index=root_index,
    )
    assert status == "ok"
    assert seen == [root_index]
    assert payload["root_index"] == root_index
    assert payload["root_seed"] == expected_seed
    assert payload["root_identity_bound"] is True
    assert list((tmp_path / "journals" / "root_0002").rglob("*.jsonl"))
...
def test_terminal_validation_covers_every_definitive_design(realized):
    ...
    definitive = {row["endpoint_id"] for row in payload["archive"]
                  if row["feasibility_level"] == "definitive"}
...
def _dual_terminal_case(sctm: float, *, strict_sctm_min: float = 0.85):
    ...
    policy = DualScTMGatePolicy(
        ...
        ancestry_sctm_min=0.70, strict_sctm_min=strict_sctm_min,
    )
    outcome = StructureOutcome(feasible=True, evaluated=True, metrics={"scTM": sctm})
    verdict = policy.evaluate(outcome)
    admission = admit_endpoint(
        F.safety_gate(), endpoint=base,
        ...
        endpoint_tokens=tuple(
            evidence.token for evidence in base.endpoint_provenance_evidence_by_pos
        ), hard_anchors=(),
    )
    authorization = authorize_ancestry(
        endpoint=base, structure_verdict=verdict,
    ...
    endpoint = dataclasses.replace(
        base, feasibility_level=level, structure_outcome=outcome,
        dual_structure_verdict=verdict, ancestry_authorization=authorization,
        endpoint_id=None,
    )
    archive = ExactArchive()
    archive.admit(endpoint, depth=0)
    return endpoint, archive, admission


def test_terminal_validation_never_relabels_dual_search_ancestry_as_final():
    provisional, provisional_archive, provisional_admission = _dual_terminal_case(0.75)
    assert provisional_archive.may_become_ancestry(provisional.endpoint_id)
    assert _terminal_records(
        types.SimpleNamespace(archive=provisional_archive),
        {provisional.endpoint_id: provisional_admission},
    ) == []

    strict, strict_archive, strict_admission = _dual_terminal_case(0.85)
    rows = _terminal_records(
        types.SimpleNamespace(archive=strict_archive),
        {strict.endpoint_id: strict_admission},
    )
    assert len(rows) == 1
    assert rows[0]["structure_definitive"] is True
    assert rows[0]["structure_feasible"] is True
    assert rows[0]["structure_metrics"]["scTM"] == 0.85

    weak_strict, weak_archive, weak_admission = _dual_terminal_case(
        0.86, strict_sctm_min=0.80,
    )
    assert weak_archive.elite() is not None  # definitive under its own weaker policy
    assert _terminal_records(
        types.SimpleNamespace(archive=weak_archive),
        {weak_strict.endpoint_id: weak_admission},
    ) == []


def _dual_result_status_case(
    sctm: float, *, root_index: int | None = 0, cap_breached: bool = False,
):
    """Build the exact three-table evidence surface used by the status classifier."""
    from inverse_folding.reference_flow.fusion_v2.structure_gate import (
    ...
    endpoint, archive, admission = _dual_terminal_case(sctm)
    base = _v2_config()
    search = cfg.V2SearchStructureConfig(
    ...
    config = dataclasses.replace(
        ...
        schedule=dataclasses.replace(
            ...
            stratum_key="highrisk_nod_v1_P1_unconstrained",
            points=(
                cfg.DepthSchedulePoint(
        ...
        projection=dataclasses.replace(
            base.projection,
            support_policy_id=HEAD_DIRECTED_CAPPED_POLICY_ID,
        ...
        safety=dataclasses.replace(base.safety, search_structure=search),
    ...
    payload = {
        "complete_endpoints": complete_endpoint_rows(
            [endpoint], depth=0, archive=archive, admission_by_endpoint=admissions,
        ),
        "archive": archive_rows(archive, admission_by_endpoint=admissions),
        "structure_evaluations": structure_evaluation_rows(
            archive, admission_by_endpoint=admissions,
        ),
    }
    ...
    cap = types.SimpleNamespace(breached=("max_logical_dfe",) if cap_breached else ())
    return config, outcome, cap, payload


def test_explicit_dual_root_with_auditable_zero_strict_pool_is_complete_negative():
    config, outcome, cap, payload = _dual_result_status_case(0.75)
    assert _ladder_result_status(
        config=config, outcome=outcome, cap_verdict=cap, payload=payload, root_index=0,
    ) == "complete_negative"


def test_complete_negative_scope_fails_closed_on_missing_identity_caps_or_evidence():
    config, outcome, cap, payload = _dual_result_status_case(0.75)

    assert _ladder_result_status(
        config=config, outcome=outcome, cap_verdict=cap, payload=payload, root_index=None,
    ) == "failed"
    assert _ladder_result_status(
        config=_v2_config(), outcome=outcome, cap_verdict=cap, payload=payload, root_index=0,
    ) == "failed"
    wrong_schedule = dataclasses.replace(
        config, schedule=dataclasses.replace(config.schedule, schedule_id="other-d2"),
    )
    assert _ladder_result_status(
        config=wrong_schedule, outcome=outcome, cap_verdict=cap, payload=payload, root_index=0,
    ) == "failed"

    outcome.exploratory_depth_override = False
    assert _ladder_result_status(
        config=config, outcome=outcome, cap_verdict=cap, payload=payload, root_index=0,
    ) == "failed"
    outcome.exploratory_depth_override = True
    payload["exploratory_depth_override"] = False
    assert _ladder_result_status(
        config=config, outcome=outcome, cap_verdict=cap, payload=payload, root_index=0,
    ) == "failed"
    payload["exploratory_depth_override"] = True

    breached = types.SimpleNamespace(breached=("max_logical_dfe",))
    assert _ladder_result_status(
        config=config, outcome=outcome, cap_verdict=breached, payload=payload, root_index=0,
    ) == "failed"

    for table in ("complete_endpoints", "archive", "structure_evaluations"):
        incomplete = {**payload, table: []}
        assert _ladder_result_status(
            config=config, outcome=outcome, cap_verdict=cap,
            payload=incomplete, root_index=0,
        ) == "failed"

    unevaluated = {
        **payload,
        **{
            name: [dict(row) for row in payload[name]]
            for name in ("complete_endpoints", "archive", "structure_evaluations")
        },
    }
    unevaluated["structure_evaluations"][0]["evaluated"] = False
    assert _ladder_result_status(
        config=config, outcome=outcome, cap_verdict=cap,
        payload=unevaluated, root_index=0,
    ) == "failed"


def test_strict_structure_that_fails_immune_admission_is_a_closed_negative_not_malformed():
    config, outcome, cap, payload = _dual_result_status_case(0.90)
    endpoint_id = payload["archive"][0]["endpoint_id"]
    outcome.best_definitive = None
    for row in payload["archive"]:
        row.update(
            feasibility_level="unvalidated", may_become_ancestry=False,
            admission_reason="cumulative immune gate failed",
            ancestry_authorization_digest=None,
        )
    for row in payload["complete_endpoints"]:
        row.update(
            feasibility_level="unvalidated", structure_evaluated=False,
            structure_feasible=False, structure_metrics_json="{}",
            ancestry_authorization_digest=None,
        )
    for row in payload["structure_evaluations"]:
        row.update(
            feasibility_level="unvalidated",
            admission_reason="cumulative immune gate failed",
            ancestry_authorization_digest=None,
        )
    assert payload["structure_evaluations"][0]["endpoint_id"] == endpoint_id
...
def test_a_strict_final_endpoint_remains_ok_and_never_counts_as_complete_negative():
    config, outcome, cap, payload = _dual_result_status_case(0.90)
    outcome.depth_reached = 0  # a normal typed projection stall may leave only the D0 pool
    assert _ladder_result_status(
        config=config, outcome=outcome, cap_verdict=cap, payload=payload, root_index=0,
    ) == "ok"

    # Legacy status semantics remain byte-for-byte/meaning compatible: without explicit dual R4
    # identity, depth-0-only strict evidence is still the pre-existing failed outcome.
    assert _ladder_result_status(
        config=_v2_config(), outcome=outcome, cap_verdict=cap,
        payload=payload, root_index=None,
    ) == "failed"
