...
campaign trips it about one launch in twenty.**

**The cleanest resolution is not to move the floor — it is to restore the surface the floor was
derived from.** Per-protein success with $k$ roots is $1-(1-p)^k$:
...
A second root removes the false-stop risk entirely and leaves the inherited floor untouched, for
twice the cells. The source campaign's single-root job over this cohort ran 4 h 45 m at 41.1 GiB and
Dual roughly doubles Head calls, so that is the price.
...
> and captures its own depth-0 roots, and nothing generated by the source campaign is read — every
> declared input is a reference sequence, checkpoint, backbone, cohort table or manifest. What the
