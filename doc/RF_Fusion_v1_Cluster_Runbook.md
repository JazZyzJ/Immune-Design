...
| `CONSTRAINT_MANIFEST` | `uricase_characterized24_active_site_manifest.yaml` | **NOT behaviour-changing here, but it corrupts the record.** The generic cohort is anchor-free (§3.1) and none of its proteins appear in that manifest, so `hard_anchor_indices` resolves to the empty set and admission is identical to unconstrained. But the path is fingerprinted into the v0 config hash and seed digest, so the artifacts of a load-bearing anchor-free run would claim a uricase active-site constraint. Pass `CONSTRAINT_MANIFEST=none` (the literal string `none` is the documented disable sentinel; an empty value still forwards the flag). Verify the launcher echoes `Constraints : none`. |
| `CONDA_ENV` | `immune-design` | correct for ailab/H200, PLI/H100, A100; the RTX PRO 6000 partition (Blackwell, sm_120) needs `immune-design-blackwell`, and the cu121 env aborts there AFTER the allocation is granted. The header also pins `--constraint=h200`, so a non-ailab partition needs `sbatch --constraint= --partition=... --gres=...` to clear it. |
