...
**P2 matched-budget — CONFIRMED design (Thinker GO; staged, NOT yet launched; ~35 GPU-h).**

**Core principle (Thinker):** the probe's "repair converges to ~same depth as explicit" (−9.47 vs −9.42) is a WARNING that H3 may be **refuted** — and the whole point of P2 is to make that negative **defensible**. The design goal is therefore NOT "make repair win" but **"give repair's novel channel full opportunity, so that if it still does not win, the conclusion is a clean simplify-to-explicit-greedy — not a you-didn't-sample-enough artifact."** This directly sets `children_per_edit`.

- **Both arms FRESH at `n_rounds=8`** (≥ both elbows: explicit ~r6, repair ~r7; matched budget — reusing P1's explicit@6 would break matched-budget).
- **Full 76 P1-ok proteins**; scTM_min 0.85, N_H 0.10, N=4, esmfold2_live; sharded ~5–6/arm (10–12 jobs).
- **`children_per_edit = 4`** — DATA-DECIDED (not the default): the repair probe's per-edit child Head-risk dispersion is LARGE (median worst−best range **0.515**; 50% of 657 edit-groups ≥0.5; 74% > ε_H; p90 9.2), i.e. best-of-2 is often far better than worst, so more children materially enrich the novel channel (best-of-K) at ~2× generation / near-zero extra refold (only shortlisted children refold). This gives the novel channel a fat sampling budget so a negative is credible.
- **Configs:** explicit arm = `rf_refine_fusion_p2_explicit.yaml` (n_rounds 8, repair off); repair arm = `rf_refine_fusion_p2_repair.yaml` (n_rounds 8, `repair.enabled=true`, `children_per_edit 4`) + `--base-if-checkpoint`/`--rf-sampler-config c1_null.yaml`.
- **Acceptance (pre-registered):** paired Wilcoxon p<0.05 (edit+repair < explicit) terminal Head at scTM≥0.85, advantage attributable to the **novel** channel (decomposed vs the ≤10% rescue). **Falsifier (defensible):** no paired margin even after the fat repair budget → Fusion simplifies to Head-guided explicit-greedy, co-adaptation claim honestly dropped.
