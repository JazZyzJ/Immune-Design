...
**Recommendation (for decision):** **Path A now** (running; harmful→neutral, reuses the `Φ`/telemetry/3-arm code; `PLAN_PLANNER_SC_GR.md` §A / `PLANNER_README.md` §A1–A2) → **C0b is the live next decisive test** (offline joint-resample headroom existence test, `PLANNER_README.md` §C0b; C0a is DONE, RAR 0021, not a NO-GO) — it gates whether the expensive **Path C** main-line rewrite is worth building. **Path B** stays C's evaluator, never a production actuator; **A3** is an optional M4-motivated intermediate (coder change, authority-bounded). The C0→C3 order holds; C-what (C2) is a sampler/commit atomic move, not a D2 logit-correction rewrite.
