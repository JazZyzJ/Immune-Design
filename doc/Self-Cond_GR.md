...
- **Path C — per-edit authority via a coordinated-proposal channel (the real lever, main line).** The binding constraint (RAR 0019 M5 / RAR 0013) is that each edit buys only ~0.2 nats within a ~3-candidate structurally-safe set, and naive widening (delta_struct/top_k/beta) is null. Lifting it needs a different **proposal form**: candidates that are **terminal-differentiated *and* structure-safe** — multi-position coordinated edits over a binding register (anchor + compensating neighbours), where single-position safe-resampling cannot reach.
...
**Recommendation (for decision):** **Path A now** (honest conservative consumption: harmful→neutral, cheap, reuses the `Φ`/telemetry/3-arm code; `PLAN_PLANNER_SC_GR.md` §A) + **Path C as the main line but *gated on C0*** — run **C0a** (existing-data structural-overlap screen) then, if it does not NO-GO, **C0b** (offline joint-resample constructive test) *before* any sampler/D2 rewrite — + **Path B as C's evaluator**, never a production actuator. `P_cheap` is an optional separate value-arm (A2, `PLAN_PLANNER_SC_GR.md` §A2), authority-bounded. The C0→C3 order is the agreed sequence; C-what (C2) is a sampler/commit atomic move, not a D2 logit-correction rewrite.
