...
- **Next decisive test — SELECTION, not cap-width (reprioritised 2026-07-01).** The original "lift `max_positions_per_block`" plan was **wrong twice**: (i) *blocked* — with `structure_topk`/`top_k=8` the block candidate count is `8^P`, so `P>4` overflows `max_candidates_per_block=4096` into a `sampled+completion-ensemble` path that `raise`s `NotImplementedError` (pos4 worked only because `8^4=4096` sits exactly on the cartesian ceiling; the "sampled path is used" claim was false); (ii) *moot* — **RAR 0023 M6** shows the 4-cap is essentially free: 54% of winning edits change ≤4 residues, and where a ≤4 candidate exists (24/35 registers) restricting to ≤4 costs **median +0.000 nat** (only ~8% of registers pay >1 nat). So the binding constraint is **which register/positions D2 selects**, not positions-per-block.
  - **Live Stage 1 = A3 (selection).** Terminal-aware actionability (`v_target_terminal_union`, `PLANNER_README.md` §A3) so D2 *selects* the high-`r_i` **low-`v_target`** registers it currently skips (C0a M4: skipped for low `b_env`, not structure) and `Φ`-ranks the right ≤4 positions within them. Coder task (crosses the §3 firewall; self-reinforcement guarded by early-freeze, like `B_sc`).
  - **Deferred fallback = widen the cap (C).** Only if A3 shows the reached registers genuinely need >4 changes: implement full-candidate-set projection for the `sampled` path (the `NotImplementedError` names it) — a localised factorized-D2 fix, not a coordinated sampler. Sized for the ~8% >1-nat tail, so low priority.
