...
# Roots are this campaign's OWN. `campaign_id` is inside the seed material
# (encoding_version, seed_schema, campaign_id, split_role, master_seed, protein_id, namespace),
# so the Dual run derives its own seeds and captures its own depth-0 roots. Nothing generated by
# the source campaign is read: every INPUT_FILE and SHARD_INPUT is a reference sequence, a
# checkpoint, a backbone, a cohort table or a manifest -- substrate DEFINITION only.
#
# ROOT_SEEDS is therefore a free, frozen choice, and its LENGTH is the roots-per-protein decision
# below. One entry = one root per protein.
ROOT_SEEDS=(20260811)           # add a second seed to run two roots per protein
...
# The frozen 100-protein cohort. All four of the source campaign's cell lists hold the SAME 100
# proteins and differ only in the prefix (verified by digest), so this reads the cohort, not a root.
COHORT_IDS=${WORK}/cell_lists/proteins.txt
sed 's/^root0_//' "${SRC}/cell_lists/root0.txt" | sort -u > "${COHORT_IDS}"

CELL_LIST=${WORK}/cell_lists/cells.txt
: > "${CELL_LIST}"
for idx in "${!ROOT_SEEDS[@]}"; do
  MASTER_SEED=${ROOT_SEEDS[$idx]}
  while read -r protein_id; do
    cell="r${idx}_${protein_id}"      # r0_, r1_ ... THIS campaign's roots, not the source's
    materialize_cell "${cell}" "${protein_id}"
    echo "${cell}" >> "${CELL_LIST}"
  done < "${COHORT_IDS}"
done
export CELL_LIST
wc -l "${CELL_LIST}"      # 100 x (number of ROOT_SEEDS)
```
...
while read -r cell; do
  python scripts/preflight_v2_canary_assembly.py \
    --cell "${WORK}/resolved_configs/${cell}.yaml=${cell#r*_}" \
...
> **The one decision this section cannot make for you: `${#ROOT_SEEDS[@]}`, roots per protein.**
> It is not "which of the source campaign's roots to reuse" — none of them is reused, and an
> earlier revision of this note framed it that way in error. A Dual root is captured by this
> campaign from this campaign's own seeds.
>
> It matters because it IS the S6 coverage question (AUDIT §4.2, §J.8). The inherited 80/100 floor
> was derived from a **4-root** surface, and per-root success among proteins that succeed at all is
> $p = 328/336 = 0.976$:
>
> | roots / protein | $\Pr[\text{protein succeeds}]$ | $E[\text{successes}]$ | $\Pr[<80]$ by chance | cells, 3 arms |
> |---:|---:|---:|---:|---:|
> | **1** | 0.9762 | 82.00 | **5.05 %** | 300 |
> | **2** | 0.9994 | 83.95 | **0.00 %** | 600 |
> | 4 | 1.0000 | 84.00 | 0.00 % | 1200 |
>
> At one root a perfectly healthy campaign falls below the inherited floor about **one launch in
> twenty**, which is why S6 is open at all. At two roots the floor is safe by a wide margin and
> needs no adjustment — the whole S6 question dissolves, for 2x the cells. The source campaign's
> single-root job over this cohort ran 4 h 45 m at 41.1 GiB, and Dual roughly doubles Head calls, so
> price the choice from there.
>
> All three arms must use the SAME `ROOT_SEEDS`. They share `campaign_id`, `split_role`,
> `master_seed` and `protein_id`, so the arms pair by construction — that is the whole basis of the
> comparison, and it holds at any number of roots.
...
Use the existing frozen high-risk 100-protein cohort, progressive D4/K12/r40, and the roots §2.2
captured for THIS campaign (`ROOT_SEEDS`; nothing of the source campaign's roots is reused). Launch
the whole cell list once per arm:
